...
psql -d tokyo2020_sch -h 127.0.0.1 -U user1 -c "\COPY place (id, name) FROM 'compose/db/master/places.csv' CSV HEADER"
psql -d tokyo2020_sch -h 127.0.0.1 -U user1 -c "\COPY olympic_schedule (id, competition_id, classification_id, title, \"begin\", \"end\", place_id, content) FROM 'compose/db/master/olympic_schedules.csv' CSV HEADER"
